...
def test_delete_product_complete_workflow(driver):
    """Test complete workflow of deleting a product"""
    login_as_admin(driver)
    driver.get(f"{BASE_URL}/admin/catalogue")
    time.sleep(2)
    ...
    product_name = f"DELETE ME Test Product {unique_id}"
    create_test_product(driver, product_name)
    time.sleep(2)
    
    # Verify product exists
    assert product_name in driver.page_source, f"Product '{product_name}' should exist before deletion"
    print(f"\n✅ Confirmed product exists: {product_name}")
    ...
    # Find all products and delete buttons
    delete_buttons = driver.find_elements(By.XPATH, "//button[text()='Delete']")
    product_names = driver.find_elements(By.CSS_SELECTOR, "p.text-lg.font-semibold, .product-name")
    
    # Find the delete button for our test product
    delete_index = -1
    ...
    if delete_index >= 0 and delete_index < len(delete_buttons):
        # Click delete button for our test product
        delete_button = delete_buttons[delete_index]
        delete_button.click()
        print(f"🗑️  Clicked Delete button for product at index {delete_index}")
        time.sleep(2)
        
        # Handle confirmation dialog/alert if present
        try:
            # Check for JavaScript confirm dialog
            alert = driver.switch_to.alert
            alert_text = alert.text
            print(f"⚠️  Confirmation dialog: {alert_text}")
            alert.accept()  # Click OK/Yes
            time.sleep(2)
        except:
            print("ℹ️  No confirmation dialog (direct deletion)")
        ...
        assert product_name not in page_source, f"Product '{product_name}' should not exist after deletion"
        ...
        assert count_after == count_before - 1, f"Product count should decrease by 1 (was {count_before}, now {count_after})"
        
        print(f"\n✅ SUCCESS: Product '{product_name}' was deleted successfully!")
    else:
        print(f"\n❌ Could not find delete button for product '{product_name}'")
        assert False, "Could not locate product's delete button"
...
def test_delete_multiple_products(driver):
    """Test deleting multiple products sequentially"""
    login_as_admin(driver)
    driver.get(f"{BASE_URL}/admin/catalogue")
    time.sleep(2)
    ...
    for i in range(3):
        unique_id = random.randint(1000, 9999)
        product_name = f"Bulk Delete Test {unique_id}"
        create_test_product(driver, product_name)
        product_names.append(product_name)
        time.sleep(2)
    
    print(f"\n📊 Created {len(product_names)} test products for deletion")
    
    # Delete each product
    deleted_count = 0
    for product_name in product_names:
        # Find and click delete button (always first one since we're deleting)
        delete_buttons = driver.find_elements(By.XPATH, "//button[text()='Delete']")
        ...
        if len(delete_buttons) > 0:
            ...
            if product_name not in driver.page_source:
                deleted_count += 1
                print(f"✅ Deleted: {product_name}")
            else:
                print(f"❌ Failed to delete: {product_name}")
    
    print(f"\n📊 Summary: Successfully deleted {deleted_count}/{len(product_names)} products")
    assert deleted_count == len(product_names), f"Should delete all {len(product_names)} products"
...
def test_delete_updates_product_count(driver):
    """Test that deleting updates the displayed product count"""
    login_as_admin(driver)
    driver.get(f"{BASE_URL}/admin/catalogue")
    time.sleep(2)
    ...
    unique_id = random.randint(1000, 9999)
    product_name = f"Count Test {unique_id}"
    create_test_product(driver, product_name)
    time.sleep(2)
    
    # Get initial count from display (if shown)
    try:
        count_display = driver.find_element(By.XPATH, "//*[contains(text(), 'Showing')]")
        initial_text = count_display.text
        print(f"\n📊 Initial count display: {initial_text}")
    except:
        print("\nℹ️  No product count display found")
    ...
    if len(delete_buttons) > 0:
        ...
        assert count_after == count_before - 1, "Product count should decrease by 1"
        print("✅ Product count updated correctly after deletion")
