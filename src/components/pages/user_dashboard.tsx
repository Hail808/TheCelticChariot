...
function formatOrderReference(orderId: number, verificationCode: string | null): string {
  if (!verificationCode) {
    return `ORD-${orderId}`; // Fallback for old orders without verification code
  }
  return `ORD-${orderId}-${verificationCode}`;
}
            ...
            guest: {
                include: {
                    ...
                    orders: {
                        ...
                        orderBy: {
                            order_date: 'desc' // Show newest first
                        }
                    },
                },
            },
                        ...
                        <tbody>
                            {orders.map((order) => (
                                <tr key={order.reference}>
                                    <td>
                                        <a href={`/user_dashboard/order/${order.reference}`}>
                                            #{order.reference}
                                        </a>
                                    </td>
                                    <td>{order.date}</td>
